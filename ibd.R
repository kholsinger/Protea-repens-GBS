...
locs <- read.csv("pop_locations.csv", header=TRUE)
...
dev.new()
p <- ggplot(for.plot, aes(x=x, y=y, color=pop, size=data)) +
     geom_point(alpha=0.8) +
     geom_point(data=means, aes(x=X, y=Y, color=pop, size=data)) +
     xlab("MDS f-dist Axis 1") +
     ylab("MDS f-dist Axis 2") +
     scale_color_manual(values=colors) +
     guides(fill=guide_legend(reverse=TRUE))
print(p)
ggsave("MDS-f-dist-repens.pdf")

for.plot <- merge(means, locs, by.x="pop", by.y="NAME")

dev.new()
par(mfrow=c(2,1))
map("worldHires", "South Africa", col="gray95", fill=TRUE,
    xlim=c(17.5,26.5), ylim=c(-35,-31))
with(for.plot, points(long, lat, col=colors.1, pch=16))
title(main="MDS f-dist Axis 1")
map("worldHires", "South Africa", col="gray95", fill=TRUE,
    xlim=c(17.5,26.5), ylim=c(-35,-31))
with(for.plot, points(long, lat, col=colors.2, pch=16))
title(main="MDS f-dist Axis 2")

dev.new()
map("worldHires", "South Africa", col="gray95", fill=TRUE,
    xlim=c(17.5,26.5), ylim=c(-35,-31))
with(for.plot, points(long, lat, col=colors.3, pch=16))
title(main="MDS f-dist Axis 1 (red) + Axis 2 (blue)")
